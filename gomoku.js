...
function init() {
	...
	var cylG = new THREE.CylinderGeometry(BALL_RADIUS/3,BALL_RADIUS/3,BALL_RADIUS*9)
	var cylM = {color: 0x77BB77};
	for (var x = 0; x < 4; x++)
		for (var z = 0; z < 4; z++) {
	...
	function make_move(m) {
		...
		for (var i = 0; i < mvs.length; i++) {
			...
			if ((this_m[0] === x) && (this_m[2] === z)) {
				if (maxi) {
					maxi = maxi[1] < this_m[1] ? this_m : maxi;
				} else {
					maxi = this_m;
				}
			}
		...
		if (maxi === undefined) {
			game_state.A.push(x + '0' + z);
			draw_balls();
			return;
		} 
		if (maxi[1] > '2') return;

		game_state.A.push('' + x + (parseInt(maxi[1]) + 1) + z);
		draw_balls();
	}
